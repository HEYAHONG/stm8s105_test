...
_I2C_Read:
	sub	sp, #9
;	driver/dht12.c: 388: disableInterrupts();
	sim
;	driver/dht12.c: 395: rdDAdr = wrDAdr+1; //读器件地址为写地址加1  
	ld	a, (0x0c, sp)
	inc	a
	ld	(0x08, sp), a
;	driver/dht12.c: 397: I2C_Start();  /*启动I2C*/  
	call	_I2C_Start
;	driver/dht12.c: 401: I2CAck = SendByte(wrDAdr); /*发写器件地址*/  
	ld	a, (0x0c, sp)
	push	a
	call	_SendByte
	addw	sp, #1
;	driver/dht12.c: 403: if(!I2CAck)  
	ld	(0x03, sp), a
	jrne	00102$
;	driver/dht12.c: 407: return I2C_ERR;  
	clr	a
	jra	00113$
00102$:
;	driver/dht12.c: 411: I2CAck = SendByte(wordAdr); /*发寄存器地址*/  
	ld	a, (0x0d, sp)
	push	a
	call	_SendByte
	addw	sp, #1
;	driver/dht12.c: 413: if(!I2CAck)  
	tnz	a
	jrne	00104$
;	driver/dht12.c: 417: return I2C_ERR;  
	clr	a
	jra	00113$
00104$:
;	driver/dht12.c: 421: I2C_Start();   /*重启I2C*/  
	call	_I2C_Start
;	driver/dht12.c: 423: I2CAck = SendByte(rdDAdr); /*发读器件地址*/  
	ld	a, (0x08, sp)
	push	a
	call	_SendByte
	addw	sp, #1
;	driver/dht12.c: 425: if(!I2CAck)  
	tnz	a
	jrne	00119$
;	driver/dht12.c: 429: return I2C_ERR;  
	clr	a
	jra	00113$
;	driver/dht12.c: 435: for(i=0;i<num-1;i++)  
00119$:
	clr	(0x09, sp)
00111$:
	clrw	x
	...
	ld	(0x02, sp), a
	clr	(0x01, sp)
;	driver/dht12.c: 439: *(pRdDat+i) = RcvByte();  
	clrw	x
	ld	a, (0x09, sp)
	ld	xl, a
	addw	x, (0x0e, sp)
...
;	driver/dht12.c: 439: *(pRdDat+i) = RcvByte();  
	...
	ldw	x, (0x04, sp)
	ld	(x), a
;	driver/dht12.c: 441: I2C_Rack();  
	call	_I2C_Rack
;	driver/dht12.c: 435: for(i=0;i<num-1;i++)  
	inc	(0x09, sp)
	jra	00111$
00107$:
;	driver/dht12.c: 445: if(i==num-1)  
	ldw	x, (0x01, sp)
	cpw	x, (0x06, sp)
	jrne	00109$
;	driver/dht12.c: 449: *(pRdDat+i) = RcvByte();  
	call	_RcvByte
	ldw	x, (0x04, sp)
	ld	(x), a
;	driver/dht12.c: 451: I2C_Rack();
	call	_I2C_Rack
;	driver/dht12.c: 452: RcvByte();
	call	_RcvByte
;	driver/dht12.c: 454: I2C_nAck();  
	call	_I2C_nAck
00109$:
;	driver/dht12.c: 458: I2C_Stop();  
	call	_I2C_Stop
;	driver/dht12.c: 460: enableInterrupts();
	rim
;	driver/dht12.c: 461: return I2C_CRR;  
	ld	a, #0x01
00113$:
;	driver/dht12.c: 463: }  
	addw	sp, #9
	ret
;	driver/dht12.c: 545: u8 i2c_read(u8 dev_addr,u8 Addr)
;	-----------------------------------------
;	 function i2c_read
;	-----------------------------------------
_i2c_read:
	push	a
;	driver/dht12.c: 548: if(I2C_Read(dev_addr,Addr,&temp,1))
	ldw	x, sp
	incw	x
	push	#0x01
	pushw	x
	...
	tnz	a
	jreq	00102$
;	driver/dht12.c: 549: return temp;
	ld	a, (0x01, sp)
;	driver/dht12.c: 550: else return 0;
	.byte 0x21
00102$:
	clr	a
00104$:
;	driver/dht12.c: 551: }
	addw	sp, #1
	ret
;	driver/dht12.c: 553: void ReadDHT12(DHT12_DATA *data) 
;	-----------------------------------------
;	 function ReadDHT12
;	-----------------------------------------
_ReadDHT12:
	sub	sp, #2
;	driver/dht12.c: 555: i2c_init();
	call	_i2c_init
;	driver/dht12.c: 556: data->W=i2c_read(0xB8,0x00);
	ldw	y, (0x05, sp)
	ldw	(0x01, sp), y
	ldw	x, y
	incw	x
	...
	popw	x
	ld	(x), a
;	driver/dht12.c: 557: data->W1=i2c_read(0xB8,0x01);
	ldw	x, (0x01, sp)
	addw	x, #0x0003
	pushw	x
	push	#0x01
	...
	popw	x
	ld	(x), a
;	driver/dht12.c: 558: data->T=i2c_read(0xB8,0x02);
	push	#0x02
	push	#0xb8
	call	_i2c_read
	addw	sp, #2
	ldw	x, (0x01, sp)
	ld	(x), a
;	driver/dht12.c: 559: data->T1=i2c_read(0xB8,0x03);
	ldw	x, (0x01, sp)
	incw	x
	pushw	x
	push	#0x03
	...
	popw	x
	ld	(x), a
;	driver/dht12.c: 560: data->sum=i2c_read(0xB8,0x04);
	ldw	x, (0x01, sp)
	addw	x, #0x0004
	pushw	x
	push	#0x04
